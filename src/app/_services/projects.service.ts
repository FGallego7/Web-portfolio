...
export class ProjectsService {
  ...
  GetProjectByTag(tags: Tag[]) {
    let filteredProjects: Project[] = [];

    this.projects.forEach(function (project) {
      // Aquí se itera sobre cada proyecto en la lista this.projects

      // Se verifica si el proyecto tiene todas las etiquetas especificadas
      if (tags.every(tag => project.tags.includes(tag))) {
        filteredProjects.push(project); // Se agrega el proyecto filtrado a la lista
      }
    });

    return filteredProjects; // Se devuelve la lista de proyectos filtrados
  }
}
